...
seedfile = r'tqqm-mode.txt'
quartfile = r'Quartic_fit-Mank_suite0.txt'
seedind = 0
mdl = dprofs.tripquad_quartictop
ndims = 12
lfh = losses.euclid_J24
pfh = priors.tripquad_quartop
toflevels = 2048
nsamples = 2
batchsize = 1
nwalkers = 24
...
def lnprob(x,n):
    ...
    P = pfh(x) + priors.gasplanet(svec, dvec, obs) + priors.saturn_rotation(x[11])
...
if fakelike:
    sampler = emcee.EnsembleSampler(nwalkers, ndims, lnfake, args=[toflevels], pool=pool)
else:
    sampler = emcee.EnsembleSampler(nwalkers, ndims, lnprob, args=[toflevels], pool=pool)
sampler.a = ascale
